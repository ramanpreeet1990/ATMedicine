...
public class Utility {
    ...
    public static boolean passwordCriteriaMatches(String newPassword) {
        Pattern alphanumberic = Pattern.compile("^(?=.*[a-zA-Z])(?=.*[0-9])");
        Matcher matcher = alphanumberic.matcher(newPassword);

        return (matcher.find() && newPassword.length() > 7 && newPassword.length() < 16);
    }
